...
if (drupalSettings.pdb.ng8.development_mode === 0) {
  enableProdMode();
}
...
export class ScrollLoader {
  ...
  public initialize(): void {
    ...
    // Defaults to body.
    // let componentsWrapper = drupalSettings.pdb.ng8.components_wrapper;
  ...
  public subscribe(): void {
    fromEvent(window, 'scroll').pipe(
      ...
      debounceTime(200)
    ).subscribe(x => this.checkElements(this.componentIds));
      ...
      if (el && this.elementInViewport(el)) {
        ...
        if (el.innerHTML.length === 0) {
          ...
          let ngClassName = (typeof this.components[id]["ngClassName"] === 'string') ?
            this.components[id]["ngClassName"] : this.convertToNgClassName(this.components[id]["element_module"]);
          let selector = '#' + id;
   ...
   */
  public bootstrapComponent(id: string, ngClassName: string, selector: string) {
    let componentFile = Drupal.url(drupalSettings.pdb.ng8.components[id]["uri"]) + '/index';

    // load and compile the module lazy loaded
    let ngModuleLoader = this.appModule.injector.get(SystemJsNgModuleLoader);

    return ngModuleLoader.load(`${componentFile}#${ngClassName}`)
      .then((moduleFactory) => this.bootstrapWithCustomSelector(moduleFactory, selector, ngClassName))
      .then(() => this.unsubscribe(id));
  ...
  public bootstrapWithCustomSelector(moduleFactory: NgModuleFactory<any>, selector: string, ngClassName: string): void {
    ...
    ngZone.run(() => {
      ...
      const lazyLoad: any = ngModule.injector.get(LazyLoadComponent);
  ...
  /**
   * Helper function to convert component name to Angular 8 ClassName.
   *
   * @param {string} - elementName in form "wu-favorites"
   * @returns {string} - ng8 class name in form "WuFavorites"
   */
